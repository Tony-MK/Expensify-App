...
const Queue_1 = require("@libs/Queue/Queue");
...
const createDownloadQueue = () => {
    const downloadItemProcessor = (item) => new Promise((resolve, reject) => {
        let downloadTimeout;
        ...
        const listenerFunction = (event, electronDownloadItem) => {
            ...
            const directory = options.directory ?? electron_1.app.getPath('downloads');
            let filePath;
            if (options.filename) {
    ...
    const queue = (0, Queue_1.default)(downloadItemProcessor);
    const enqueueDownloadItem = (item) => {
        queue.enqueue(item);
    };
