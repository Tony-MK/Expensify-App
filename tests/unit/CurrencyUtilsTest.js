...
const CurrencyUtils = require("@src/libs/CurrencyUtils");
...
const currencyCodeList = Object.keys(currencyList_json_1.default);
const AVAILABLE_LOCALES = [CONST_1.default.LOCALES.EN, CONST_1.default.LOCALES.ES];
describe('CurrencyUtils', () => {
    beforeAll(() => {
        react_native_onyx_1.default.init({
            ...
            initialKeyStates: {
                [ONYXKEYS_1.default.NVP_PREFERRED_LOCALE]: CONST_1.default.LOCALES.DEFAULT,
    ...
    describe('getLocalizedCurrencySymbol', () => {
        test.each(AVAILABLE_LOCALES)('Returns non empty string for all currencyCode with preferredLocale %s', (preferredLocale) => IntlStore_1.default.load(preferredLocale).then(() => {
    ...
    describe('convertToFrontendAmountAsString', () => {
        test.each([
            ...
            expect(CurrencyUtils.convertToFrontendAmountAsString(input, currency ?? CONST_1.default.CURRENCY.USD)).toBe(expectedResult);
        });
    });
    describe('convertToDisplayString', () => {
        ...
        ])('Correctly displays %s in ES locale', (currency, amount, expectedResult) => IntlStore_1.default.load(CONST_1.default.LOCALES.ES).then(() => expect(CurrencyUtils.convertToDisplayString(amount, currency)).toBe(expectedResult)));
    });
    describe('convertToShortDisplayString', () => {
        ...
        ])('Correctly displays %s in ES locale', (currency, amount, expectedResult) => IntlStore_1.default.load(CONST_1.default.LOCALES.ES).then(() => expect(CurrencyUtils.convertToShortDisplayString(amount, currency)).toBe(expectedResult)));
    });
});
