...
const memoize_1 = require("../../src/libs/memoize");
describe('memoize test', () => {
    it('should return the memoized result', () => {
        const add = (a, b) => a + b;
        const memoizedAdd = (0, memoize_1.default)(add);
    ...
    it('should not call original function if the same arguments are passed', () => {
        const fn = jest.fn();
        const memoizedFn = (0, memoize_1.default)(fn);
        memoizedFn(2, 3);
        memoizedFn(2, 3);
        memoizedFn(2, 3);
    ...
    it('should handle deep comparison', () => {
        const fn = jest.fn();
        const memoizedFn = (0, memoize_1.default)(fn, { equality: 'deep' });
    ...
    it('should handle custom key comparator', () => {
        const fn = jest.fn();
        const memoizedFn = (0, memoize_1.default)(fn, { equality: () => true });
        memoizedFn(1, 2);
        memoizedFn(1, 3);
        memoizedFn(1, 4);
        memoizedFn(1, 5);
    ...
    it('should handle cache eviction', () => {
        const fn = jest.fn();
        const memoizedFn = (0, memoize_1.default)(fn, { maxSize: 1 });
        memoizedFn(1, 2);
        memoizedFn(1, 3);
        memoizedFn(1, 2);
        memoizedFn(1, 3);
