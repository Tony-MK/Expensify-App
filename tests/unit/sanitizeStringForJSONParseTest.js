...
const sanitizeStringForJSONParse_1 = require("../../.github/libs/sanitizeStringForJSONParse");
...
describe('sanitizeStringForJSONParse', () => {
    describe.each(badInputs)('willDetectBadInputs', (input) => {
        test('sanitizeStringForJSONParse', () => {
            expect(() => (0, sanitizeStringForJSONParse_1.default)(input)).toThrow();
        });
    });
    describe.each(invalidJSONData)('canHandleInvalidJSON', (input, expectedOutput) => {
        test('sanitizeStringForJSONParse', () => {
            ...
            const goodJSON = JSON.parse(`{"key": "${(0, sanitizeStringForJSONParse_1.default)(input)}"}`);
