...
const AppImport = require("@userActions/App");
// eslint-disable-next-line no-restricted-syntax -- this is required to allow mocking
const OnyxUpdateManager = require("@userActions/OnyxUpdateManager");
// eslint-disable-next-line no-restricted-syntax -- this is required to allow mocking
const OnyxUpdateManagerUtilsImport = require("@userActions/OnyxUpdateManager/utils");
// eslint-disable-next-line no-restricted-syntax -- this is required to allow mocking
const ApplyUpdatesImport = require("@userActions/OnyxUpdateManager/utils/applyUpdates");
// eslint-disable-next-line no-restricted-syntax -- this is required to allow mocking
const OnyxUpdatesImport = require("@userActions/OnyxUpdates");
...
const OnyxUpdates = OnyxUpdatesImport;
const App = AppImport;
const ApplyUpdates = ApplyUpdatesImport;
const OnyxUpdateManagerUtils = OnyxUpdateManagerUtilsImport;
const update2 = OnyxUpdateMockUtils_1.default.createUpdate(2);
const pendingUpdateUpTo2 = OnyxUpdateMockUtils_1.default.createPendingUpdate(2);
const update3 = OnyxUpdateMockUtils_1.default.createUpdate(3);
const pendingUpdateUpTo3 = OnyxUpdateMockUtils_1.default.createPendingUpdate(3);
const offsetUpdate3 = OnyxUpdateMockUtils_1.default.createUpdate(3, undefined, 1);
const update4 = OnyxUpdateMockUtils_1.default.createUpdate(4);
const update5 = OnyxUpdateMockUtils_1.default.createUpdate(5);
const update6 = OnyxUpdateMockUtils_1.default.createUpdate(6);
const update7 = OnyxUpdateMockUtils_1.default.createUpdate(7);
const update8 = OnyxUpdateMockUtils_1.default.createUpdate(8);
describe('OnyxUpdateManager', () => {
    ...
    it('should only apply deferred updates that are newer than the last locally applied update (pending deferred updates)', async () => {
        ...
        OnyxUpdateManagerUtils.mockValues.beforeValidateAndApplyDeferredUpdates = async () => {
            ...
            await react_native_onyx_1.default.set(ONYXKEYS_1.default.ONYX_UPDATES_LAST_UPDATE_ID_APPLIED_TO_CLIENT, 5);
            OnyxUpdateManagerUtils.mockValues.beforeValidateAndApplyDeferredUpdates = undefined;
        };
        return OnyxUpdateManager.queryPromise.then(() => {
            // After all missing and deferred updates have been applied, the lastUpdateIDAppliedToClient should be 6.
    ...
    it('should re-fetch missing updates if the lastUpdateIDFromClient has increased, but there are still gaps after the locally applied update', async () => {
        ...
        ApplyUpdates.mockValues.beforeApplyUpdates = async () => {
            ...
            // When this happens, the OnyxUpdateManager should trigger another validation of the deferred updates,
            // without triggering another re-fetching of missing updates from the server.
            await react_native_onyx_1.default.set(ONYXKEYS_1.default.ONYX_UPDATES_LAST_UPDATE_ID_APPLIED_TO_CLIENT, 4);
            ApplyUpdates.mockValues.beforeApplyUpdates = undefined;
        };
        return OnyxUpdateManager.queryPromise.then(() => {
            // After all missing and deferred updates have been applied, the lastUpdateIDAppliedToClient should be 6.
