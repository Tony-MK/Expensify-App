...
const getPreviousVersion_1 = require("@github/actions/javascript/getPreviousVersion/getPreviousVersion");
...
const DUMMY_DIR = path_1.default.resolve(os_1.default.homedir(), 'DumDumRepo');
const GIT_REMOTE = path_1.default.resolve(os_1.default.homedir(), 'dummyGitRemotes/DumDumRepo');
...
function setupGitAsOSBotify() {
    ...
    exec(`git config --local user.name ${CONST_1.default.OS_BOTIFY}`);
...
function initGithubAPIMocking() {
    ...
    jest.spyOn(GithubUtils_1.default.octokit.repos, 'compareCommits').mockImplementation((params) => {
...
function bumpVersion(level, isRemote = false) {
...
function createBasicPR(num) {
    ...
    const filePath = path_1.default.resolve(process.cwd(), `PR${num}.txt`);
...
function cherryPickPRToStaging(num, resolveVersionBumpConflicts = () => { }, resolveMergeCommitConflicts = () => { }) {
    ...
    const prMergeCommit = (0, child_process_1.execSync)('git rev-parse HEAD', { encoding: 'utf-8' }).trim();
    bumpVersion(VersionUpdater.SEMANTIC_VERSION_LEVELS.BUILD);
    const versionBumpCommit = (0, child_process_1.execSync)('git rev-parse HEAD', { encoding: 'utf-8' }).trim();
    checkoutRepo();
    setupGitAsOSBotify();
    mockGetInput.mockReturnValue(VersionUpdater.SEMANTIC_VERSION_LEVELS.PATCH);
    const previousPatchVersion = (0, getPreviousVersion_1.default)();
...
function cherryPickPRToProduction(num, resolveVersionBumpConflicts = () => { }, resolveMergeCommitConflicts = () => { }) {
    ...
    let versionBumpCommit = (0, child_process_1.execSync)('git rev-parse HEAD', { encoding: 'utf-8' }).trim();
    checkoutRepo();
    setupGitAsOSBotify();
...
async function assertPRsMergedBetween(from, to, expected) {
    checkoutRepo();
    const PRs = await GitUtils_1.default.getPullRequestsDeployedBetween(from, to, CONST_1.default.APP_REPO);
...
`;
    ...
    test('Manual version bump', async () => {
        ...
        const packageJSONBefore = fs_1.default.readFileSync('package.json', { encoding: 'utf-8' });
        mergePR(15);
        cherryPickPRToStaging(15, () => {
