...
const reportCollectionDataSet = {
    [`${ONYXKEYS_1.default.COLLECTION.REPORT}${FAKE_NEW_REPORT_ID}`]: newReport,
    [`${ONYXKEYS_1.default.COLLECTION.REPORT}${FAKE_SELF_DM_REPORT_ID}`]: selfDM,
};
describe('Transaction', () => {
    beforeAll(() => {
        react_native_onyx_1.default.init({
            ...
            initialKeyStates: {
                [ONYXKEYS_1.default.SESSION]: { accountID: CURRENT_USER_ID },
    ...
    describe('changeTransactionsReport', () => {
        it('correctly moves the IOU report action when an unreported transaction is added to an expense report', async () => {
            ...
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.REPORT_ACTIONS}${FAKE_SELF_DM_REPORT_ID}`, { [oldIOUAction.reportActionID]: oldIOUAction });
            (0, Transaction_1.changeTransactionsReport)([transaction.transactionID], FAKE_NEW_REPORT_ID, false, CURRENT_USER_ID, 'test@example.com');
            ...
            const reportActions = await new Promise((resolve) => {
                const connection = react_native_onyx_1.default.connect({
                    key: `${ONYXKEYS_1.default.COLLECTION.REPORT_ACTIONS}${FAKE_NEW_REPORT_ID}`,
                    callback: (value) => {
            ...
            expect((0, ReportActionsUtils_1.getIOUActionForTransactionID)(Object.values(reportActions ?? {}), transaction.transactionID)).toBeDefined();
        });
        it('correctly moves the IOU report action when a transaction is moved from one expense report to another', async () => {
            ...
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.TRANSACTION}${transaction.transactionID}`, transaction);
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.REPORT_ACTIONS}${FAKE_OLD_REPORT_ID}`, { [oldIOUAction.reportActionID]: oldIOUAction });
            (0, Transaction_1.changeTransactionsReport)([transaction.transactionID], FAKE_NEW_REPORT_ID, false, CURRENT_USER_ID, 'test@example.com');
            await (0, waitForBatchedUpdates_1.default)();
            const reportActions = await new Promise((resolve) => {
                const connection = react_native_onyx_1.default.connect({
        ...
        it('correctly handles reportNextStep parameter when moving transactions between reports', async () => {
            ...
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.TRANSACTION}${transaction.transactionID}`, transaction);
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.REPORT_ACTIONS}${FAKE_OLD_REPORT_ID}`, { [oldIOUAction.reportActionID]: oldIOUAction });
            (0, Transaction_1.changeTransactionsReport)([transaction.transactionID], FAKE_NEW_REPORT_ID, false, CURRENT_USER_ID, 'test@example.com', undefined, mockReportNextStep);
            await (0, waitForBatchedUpdates_1.default)();
            ...
            const nextStepFailureData = failureData?.find((data) => data.key === `${ONYXKEYS_1.default.COLLECTION.NEXT_STEP}${FAKE_NEW_REPORT_ID}`);
        ...
        it('correctly handles reportNextStep parameter when moving transactions to unreported report', async () => {
            ...
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.TRANSACTION}${transaction.transactionID}`, transaction);
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.REPORT_ACTIONS}${FAKE_OLD_REPORT_ID}`, { [oldIOUAction.reportActionID]: oldIOUAction });
            (0, Transaction_1.changeTransactionsReport)([transaction.transactionID], CONST_1.default.REPORT.UNREPORTED_REPORT_ID, false, CURRENT_USER_ID, 'test@example.com', undefined, mockReportNextStep);
            await (0, waitForBatchedUpdates_1.default)();
            expect(mockAPIWrite).toHaveBeenCalled();
            const apiWriteCall = mockAPIWrite.mock.calls.at(0);
            const failureData = apiWriteCall?.[2]?.failureData;
            const nextStepFailureData = failureData?.find((data) => data.key === `${ONYXKEYS_1.default.COLLECTION.NEXT_STEP}${CONST_1.default.REPORT.UNREPORTED_REPORT_ID}`);
            expect(nextStepFailureData).toBeDefined();
            expect(nextStepFailureData?.value).toEqual(mockReportNextStep);
            mockAPIWrite.mockRestore();
        });
        it('correctly handles undefined reportNextStep parameter', async () => {
            ...
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.TRANSACTION}${transaction.transactionID}`, transaction);
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.REPORT_ACTIONS}${FAKE_OLD_REPORT_ID}`, { [oldIOUAction.reportActionID]: oldIOUAction });
            (0, Transaction_1.changeTransactionsReport)([transaction.transactionID], FAKE_NEW_REPORT_ID, false, CURRENT_USER_ID, 'test@example.com', undefined, undefined);
            await (0, waitForBatchedUpdates_1.default)();
            expect(mockAPIWrite).toHaveBeenCalled();
            const apiWriteCall = mockAPIWrite.mock.calls.at(0);
            const failureData = apiWriteCall?.[2]?.failureData;
        ...
        it('correctly handles ASAP submit beta enabled when moving transactions', async () => {
            ...
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.TRANSACTION}${transaction.transactionID}`, transaction);
            await react_native_onyx_1.default.merge(`${ONYXKEYS_1.default.COLLECTION.REPORT_ACTIONS}${FAKE_OLD_REPORT_ID}`, { [oldIOUAction.reportActionID]: oldIOUAction });
            (0, Transaction_1.changeTransactionsReport)([transaction.transactionID], FAKE_NEW_REPORT_ID, true, CURRENT_USER_ID, 'test@example.com');
            await (0, waitForBatchedUpdates_1.default)();
            expect(mockAPIWrite).toHaveBeenCalled();
            const apiWriteCall = mockAPIWrite.mock.calls.at(0);
            const parameters = apiWriteCall?.[1];
        ...
        it('correctly handles different account IDs and emails when moving transactions', async () => {
            ...
            (0, Transaction_1.changeTransactionsReport)([transaction.transactionID], FAKE_NEW_REPORT_ID, false, customAccountID, customEmail);
            await (0, waitForBatchedUpdates_1.default)();
            expect(mockAPIWrite).toHaveBeenCalled();
            const apiWriteCall = mockAPIWrite.mock.calls.at(0);
            const parameters = apiWriteCall?.[1];
