...
const Reauthentication_1 = require("@libs/Middleware/Reauthentication");
const CONST_1 = require("@src/CONST");
const NetworkActions = require("@src/libs/actions/Network");
...
const PersonalDetails = require("@src/libs/actions/PersonalDetails");
...
const Network = require("@src/libs/Network");
...
const NetworkConnection_1 = require("@src/libs/NetworkConnection");
...
beforeEach(() => {
    ...
    return Promise.all([SequentialQueue.waitForIdle(), (0, waitForBatchedUpdates_1.default)(), PersistedRequests.clear(), Onyx.clear()]).then(() => {
...
describe('NetworkTests', () => {
    test('failing to reauthenticate should not log out user', () => {
        ...
        return TestHelper.signInWithTestUser(TEST_USER_ACCOUNT_ID, TEST_USER_LOGIN)
            ...
            const mockedXhr = jest
                ...
                .mockImplementationOnce(() => Promise.reject(new Error(CONST_1.default.ERROR.FAILED_TO_FETCH)))
                .mockImplementationOnce(() => Promise.resolve({
            ...
            const callsToAuthenticate = HttpUtils_1.default.xhr.mock.calls.filter(([command]) => command === 'Authenticate');
    ...
    test('failing to reauthenticate while offline should not log out user', async () => {
        ...
        await Onyx.set(ONYXKEYS_1.default.HAS_LOADED_APP, true);
        ...
        await Onyx.set(ONYXKEYS_1.default.NETWORK, { isOffline: false });
        ...
        await Onyx.set(ONYXKEYS_1.default.NETWORK, { isOffline: true });
        await Onyx.set(ONYXKEYS_1.default.NETWORK, { isOffline: false });
        ...
        await (0, waitForBatchedUpdates_1.default)(); // Now we wait for all updates after the auth request fails
    ...
    test('consecutive API calls eventually succeed when authToken is expired', () => {
        ...
        const TEST_USER_LOGIN = 'test@testguy.com';
        const TEST_USER_ACCOUNT_ID = 1;
        const reconnectionCallbacksSpy = jest.spyOn(NetworkConnection_1.default, 'triggerReconnectionCallbacks');
        ...
        return TestHelper.signInWithTestUser(TEST_USER_ACCOUNT_ID, TEST_USER_LOGIN)
            ...
            const callsToOpenPublicProfilePage = HttpUtils_1.default.xhr.mock.calls.filter(([command]) => command === 'OpenPublicProfilePage');
            const callsToAuthenticate = HttpUtils_1.default.xhr.mock.calls.filter(([command]) => command === 'Authenticate');
    ...
    test('Request will not run until credentials are read from Onyx', () => {
        ...
        const spyHttpUtilsXhr = jest.spyOn(HttpUtils_1.default, 'xhr').mockImplementation(() => Promise.resolve({}));
    ...
    test('Non-retryable request will not be retried if connection is lost in flight', () => {
        ...
        const xhr = jest.spyOn(HttpUtils_1.default, 'xhr').mockImplementationOnce(() => {
    ...
    test('test Bad Gateway status will log hmmm', () => {
        ...
        const logHmmmSpy = jest.spyOn(Log_1.default, 'hmmm');
    ...
    test('test unknown status will log alert', () => {
        ...
        const logAlertSpy = jest.spyOn(Log_1.default, 'alert');
        // Given we have a request made while online
        return Onyx.set(ONYXKEYS_1.default.NETWORK, { isOffline: false })
            .then(() => {
            Network.post('MockBadNetworkResponse', { param1: 'value1' });
    ...
    test('test Failed to fetch error for non-retryable requests resolve with unable to retry jsonCode', () => {
        ...
        return Onyx.set(ONYXKEYS_1.default.NETWORK, { isOffline: false })
            ...
            expect(response?.jsonCode).toBe(CONST_1.default.JSON_CODE.UNABLE_TO_RETRY);
        });
    });
    ...
    test('poor connection simulation', async () => {
        const logSpy = jest.spyOn(Log_1.default, 'info');
