...
const shouldPreventDeeplinkPrompt_1 = require("@libs/Navigation/helpers/shouldPreventDeeplinkPrompt");
...
function promptToOpenInDesktopApp(currentUserAccountID, initialUrl = '') {
    ...
    if (expensify_common_1.Str.startsWith(window.location.pathname, expensify_common_1.Str.normalizeUrl(ROUTES_1.default.TRANSITION_BETWEEN_APPS))) {
        ...
        const isMagicLink = CONST_1.default.REGEX.ROUTES.VALIDATE_LOGIN.test(window.location.pathname);
...
function DeeplinkWrapper({ children, isAuthenticated, autoAuthState, initialUrl }) {
    const [currentScreen, setCurrentScreen] = (0, react_1.useState)();
    const [hasShownPrompt, setHasShownPrompt] = (0, react_1.useState)(false);
    const removeListener = (0, react_1.useRef)(undefined);
    const [isActingAsDelegate] = (0, useOnyx_1.default)(ONYXKEYS_1.default.ACCOUNT, {
        selector: (account) => !!account?.delegatedAccess?.delegate,
        canBeMissing: true,
    });
    const [currentUserAccountID] = (0, useOnyx_1.default)(ONYXKEYS_1.default.SESSION, {
        selector: (session) => session?.accountID,
        canBeMissing: true,
    });
    const isActingAsDelegateRef = (0, react_1.useRef)(isActingAsDelegate);
    const delegatorEmailRef = (0, react_1.useRef)((0, Url_1.getSearchParamFromUrl)((0, currentUrl_1.default)(), 'delegatorEmail'));
    (0, react_1.useEffect)(() => {
        ...
        if (isAuthenticated === false) {
            ...
            Navigation_1.default.isNavigationReady().then(() => {
                // Get initial route
                const initialRoute = navigationRef_1.default.current?.getCurrentRoute();
                setCurrentScreen(initialRoute?.name);
                removeListener.current = navigationRef_1.default.current?.addListener('state', (event) => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        const routeRegex = new RegExp(CONST_1.default.REGEX.ROUTES.UNLINK_LOGIN);
        const isUnsupportedDeeplinkRoute = routeRegex.test(window.location.pathname);
        const route = window.location.pathname.replace('/', '');
        const isConnectionCompleteRoute = route === ROUTES_1.default.CONNECTION_COMPLETE || route === ROUTES_1.default.BANK_CONNECTION_COMPLETE;
        ...
        if (isAuthenticated) {
            ...
            const preventPrompt = (0, shouldPreventDeeplinkPrompt_1.default)(currentScreen);
