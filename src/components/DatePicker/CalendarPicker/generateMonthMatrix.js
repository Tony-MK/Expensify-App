...
function generateMonthMatrix(year, month) {
    ...
    const weekEndsOn = DateUtils_1.default.getWeekEndsOn();
    // Get the number of days in the month and the first day of the month
    const firstDayOfMonth = (0, date_fns_1.startOfMonth)(new Date(year, month, 1));
    const daysInMonth = (0, date_fns_1.getDaysInMonth)(firstDayOfMonth);
    // Create a matrix to hold the calendar days
    const matrix = [];
    let currentWeek = [];
    // Add calendar days to the matrix
    for (let i = 1; i <= daysInMonth; i++) {
        const currentDate = (0, date_fns_1.addDays)(firstDayOfMonth, i - 1);
