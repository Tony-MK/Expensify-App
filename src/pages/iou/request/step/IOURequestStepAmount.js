...
function IOURequestStepAmount(_a) {
    ...
    var iouRequestType = (0, TransactionUtils_1.getRequestType)(transaction, isBetaEnabled(CONST_1.default.BETAS.MANUAL_DISTANCE));
    ...
    var transactionAmount = ((_b = (0, ReportUtils_1.getTransactionDetails)(currentTransaction)) !== null && _b !== void 0 ? _b : { amount: 0 }).amount;
    ...
    return (<StepScreenWrapper_1.default headerTitle={translate('iou.amount')} onBackButtonPress={navigateBack} testID={IOURequestStepAmount.displayName} shouldShowWrapper={!!backTo || isEditing} includeSafeAreaPaddingBottom shouldShowNotFoundPage={shouldShowNotFoundPage}>
            <MoneyRequestAmountForm_1.default isEditing={!!backTo || isEditing} currency={currency} amount={Math.abs(transactionAmount)} skipConfirmation={shouldSkipConfirmation !== null && shouldSkipConfirmation !== void 0 ? shouldSkipConfirmation : false} iouType={iouType} policyID={policy === null || policy === void 0 ? void 0 : policy.id} ref={function (e) {
            textInput.current = e;
        }} shouldKeepUserInput={transaction === null || transaction === void 0 ? void 0 : transaction.shouldShowOriginalAmount} onCurrencyButtonPress={navigateToCurrencySelectionPage} onSubmitButtonPress={saveAmountAndCurrency} selectedTab={iouRequestType} chatReportID={reportID}/>
