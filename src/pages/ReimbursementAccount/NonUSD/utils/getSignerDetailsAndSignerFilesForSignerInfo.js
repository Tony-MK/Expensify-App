...
const { FULL_NAME, EMAIL, JOB_TITLE, DATE_OF_BIRTH, ADDRESS, STREET, CITY, STATE, ZIP_CODE, PROOF_OF_DIRECTORS, ADDRESS_PROOF, COPY_OF_ID, CODICE_FISCALE, DOWNLOADED_PDS_AND_FSG } = CONST_1.default.NON_USD_BANK_ACCOUNT.SIGNER_INFO_STEP.SIGNER_INFO_DATA;
const { PREFIX: BENEFICIAL_PREFIX, FIRST_NAME, LAST_NAME, DOB, STREET: BENEFICIAL_STREET, CITY: BENEFICIAL_CITY, STATE: BENEFICIAL_STATE, ZIP_CODE: BENEFICIAL_ZIP_CODE, } = CONST_1.default.NON_USD_BANK_ACCOUNT.BENEFICIAL_OWNER_INFO_STEP.BENEFICIAL_OWNER_DATA;
...
function getSignerDetailsAndSignerFilesForSignerInfo(reimbursementAccountDraft, signerEmail, isUserBeneficialOwner) {
    ...
    if (isUserBeneficialOwner) {
        ...
        beneficialOwnerFields.forEach((fieldName) => {
            const beneficialFieldKey = `${BENEFICIAL_PREFIX}_${CONST_1.default.NON_USD_BANK_ACCOUNT.CURRENT_USER_KEY}_${fieldName}`;
