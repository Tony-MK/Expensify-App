...
function Plaid({ onNext, setUSDBankAccountStep }) {
    ...
    return (<FormProvider_1.default formID={ONYXKEYS_1.default.FORMS.REIMBURSEMENT_ACCOUNT_FORM} validate={BankAccounts_1.validatePlaidSelection} onSubmit={handleNextPress} scrollContextEnabled submitButtonText={translate('common.next')} style={[styles.mh5, styles.flexGrow1]} isSubmitButtonVisible={(plaidData?.bankAccounts ?? []).length > 0} shouldHideFixErrorsAlert>
            <InputWrapper_1.default InputComponent={AddPlaidBankAccount_1.default} text={translate('bankAccount.plaidBodyCopy')} onSelect={(plaidAccountID) => {
            (0, ReimbursementAccount_1.updateReimbursementAccountDraft)({ plaidAccountID });
