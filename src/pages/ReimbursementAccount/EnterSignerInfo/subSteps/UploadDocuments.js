...
function UploadDocuments({ onNext, isEditing, policyID }) {
    ...
    const isPDSandFSGDownloaded = enterSignerInfoFormDraft?.[EnterSignerInfoForm_1.default.DOWNLOADED_PDS_AND_FSG] ?? false;
    const [isPDSandFSGDownloadedTouched, setIsPDSandFSGDownloadedTouched] = (0, react_1.useState)(false);
    const copyOfIDInputID = EnterSignerInfoForm_1.default.SIGNER_COPY_OF_ID;
    const addressProofInputID = EnterSignerInfoForm_1.default.SIGNER_ADDRESS_PROOF;
    const directorsProofInputID = EnterSignerInfoForm_1.default.PROOF_OF_DIRECTORS;
    const codiceFiscaleInputID = EnterSignerInfoForm_1.default.SIGNER_CODICE_FISCALE;
    const defaultValues = {
    ...
    const handleRemoveFile = (fileName, uploadedFiles, inputID, setFiles) => {
        ...
        (0, FormActions_1.setDraftValues)(ONYXKEYS_1.default.FORMS.ENTER_SINGER_INFO_FORM, { [inputID]: newUploadedIDs });
        setFiles(newUploadedIDs);
    };
    const handleSelectFile = (files, uploadedFiles, inputID, setFiles) => {
        (0, FormActions_1.setDraftValues)(ONYXKEYS_1.default.FORMS.ENTER_SINGER_INFO_FORM, { [inputID]: [...uploadedFiles, ...files] });
    ...
    const setUploadError = (error, inputID) => {
        ...
        (0, FormActions_1.setErrorFields)(ONYXKEYS_1.default.FORMS.ENTER_SINGER_INFO_FORM, { [inputID]: { onUpload: error } });
    };
    const handleDownload = () => {
        (0, Link_1.openExternalLink)(`${environmentUrl}/pdfs/PDSAndFSG.pdf`);
        setIsPDSandFSGDownloadedTouched(true);
        (0, FormActions_1.setDraftValues)(ONYXKEYS_1.default.FORMS.ENTER_SINGER_INFO_FORM, { [EnterSignerInfoForm_1.default.DOWNLOADED_PDS_AND_FSG]: true });
    };
    return (<FormProvider_1.default formID={ONYXKEYS_1.default.FORMS.ENTER_SINGER_INFO_FORM} submitButtonText={translate(isEditing ? 'common.confirm' : 'common.next')} onSubmit={handleSubmitWithDownload} validate={validate} style={[styles.mh5, styles.flexGrow1]} submitButtonStyles={[styles.mb0]}>
            ...
            }} acceptedFileTypes={[...CONST_1.default.ENTER_SIGNER_INFO.ALLOWED_FILE_TYPES]} value={uploadedCodiceFiscale} inputID={codiceFiscaleInputID} setError={(error) => {
            ...
            {isDocumentNeededStatus.isPRDAndFSGNeeded && (<react_native_1.View style={[styles.alignItemsStart]}>
                    ...
                    {!isPDSandFSGDownloaded && isPDSandFSGDownloadedTouched && (<DotIndicatorMessage_1.default style={[styles.formError, styles.mt3]} type="error" messages={{ [EnterSignerInfoForm_1.default.DOWNLOADED_PDS_AND_FSG]: translate('common.error.fieldRequired') }}/>)}
