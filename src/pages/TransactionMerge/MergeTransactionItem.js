...
function MergeTransactionItem({ item, isFocused, showTooltip, isDisabled, onFocus, shouldSyncFocus, onSelectRow }) {
    ...
    return (<OfflineWithFeedback_1.default pendingAction={item.pendingAction}>
            ...
            onSelectRow(item);
        }} disabled={isDisabled && !item.isSelected} accessibilityLabel={item.text ?? ''} role={(0, utils_1.getButtonRole)(true)} isNested onMouseDown={(e) => e.preventDefault()} hoverStyle={[!item.isDisabled && styles.hoveredComponentBG]} dataSet={{ [CONST_1.default.SELECTION_SCRAPER_HIDDEN_ELEMENT]: true, [CONST_1.default.INNER_BOX_SHADOW_ELEMENT]: false }} id={item.keyForList ?? ''} style={[
