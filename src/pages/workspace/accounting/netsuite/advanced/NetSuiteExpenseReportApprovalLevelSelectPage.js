...
function NetSuiteExpenseReportApprovalLevelSelectPage({ policy }) {
    const { translate } = (0, useLocalize_1.default)();
    const policyID = policy?.id;
    const styles = (0, useThemeStyles_1.default)();
    const config = policy?.connections?.netsuite?.options.config;
    const data = Object.values(CONST_1.default.NETSUITE_REPORTS_APPROVAL_LEVEL).map((approvalType) => ({
        value: approvalType,
    ...
    const selectExpenseReportApprovalLevel = (0, react_1.useCallback)((row) => {
        if (row.value !== config?.syncOptions.exportReportsTo && policyID) {
            (0, NetSuiteCommands_1.updateNetSuiteExportReportsTo)(policyID, row.value, config?.syncOptions.exportReportsTo ?? CONST_1.default.NETSUITE_REPORTS_APPROVAL_LEVEL.REPORTS_APPROVED_NONE);
        }
