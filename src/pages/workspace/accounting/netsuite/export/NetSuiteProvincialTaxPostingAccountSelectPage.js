...
function NetSuiteProvincialTaxPostingAccountSelectPage({ policy }) {
    ...
    const netsuiteTaxAccountOptions = (0, react_1.useMemo)(() => (0, PolicyUtils_1.getNetSuiteTaxAccountOptions)(policy ?? undefined, selectedSubsidiary?.country, config?.provincialTaxPostingAccount), [config?.provincialTaxPostingAccount, policy, selectedSubsidiary?.country]);
    ...
    const updateTaxAccount = (0, react_1.useCallback)(({ value }) => {
        if (config?.provincialTaxPostingAccount !== value && policyID) {
            (0, NetSuiteCommands_1.updateNetSuiteProvincialTaxPostingAccount)(policyID, value, config?.provincialTaxPostingAccount);
        }
    ...
    return (<SelectionScreen_1.default policyID={policyID} accessVariants={[CONST_1.default.POLICY.ACCESS_VARIANTS.ADMIN, CONST_1.default.POLICY.ACCESS_VARIANTS.CONTROL]} featureName={CONST_1.default.POLICY.MORE_FEATURES.ARE_CONNECTIONS_ENABLED} displayName={NetSuiteProvincialTaxPostingAccountSelectPage.displayName} sections={netsuiteTaxAccountOptions.length ? [{ data: netsuiteTaxAccountOptions }] : []} listItem={RadioListItem_1.default} onSelectRow={updateTaxAccount} initiallyFocusedOptionKey={initiallyFocusedOptionKey} onBackButtonPress={() => Navigation_1.default.goBack(ROUTES_1.default.POLICY_ACCOUNTING_NETSUITE_EXPORT.getRoute(policyID))} title="workspace.netsuite.journalEntriesProvTaxPostingAccount" listEmptyContent={listEmptyContent} connectionName={CONST_1.default.POLICY.CONNECTIONS.NAME.NETSUITE} shouldBeBlocked={!!config?.suiteTaxEnabled || !config?.syncOptions.syncTax || !(0, PolicyUtils_1.canUseProvincialTaxNetSuite)(selectedSubsidiary?.country)} pendingAction={(0, PolicyUtils_1.settingsPendingAction)([CONST_1.default.NETSUITE_CONFIG.PROVINCIAL_TAX_POSTING_ACCOUNT], config?.pendingFields)} errors={(0, ErrorUtils_1.getLatestErrorField)(config, CONST_1.default.NETSUITE_CONFIG.PROVINCIAL_TAX_POSTING_ACCOUNT)} errorRowStyles={[styles.ph5, styles.pv3]} onClose={() => (0, Policy_1.clearNetSuiteErrorField)(policyID, CONST_1.default.NETSUITE_CONFIG.PROVINCIAL_TAX_POSTING_ACCOUNT)}/>);
}
