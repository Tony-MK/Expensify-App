...
const STEP_FIELDS = [NetSuiteCustomFieldForm_1.default.SEGMENT_NAME];
function CustomSegmentNameStep({ customSegmentType, onNext, isEditing, customSegments }) {
    ...
    const validate = (0, react_1.useCallback)((values) => {
        ...
        else if (customSegments?.find((customSegment) => customSegment.segmentName.toLowerCase() === values[NetSuiteCustomFieldForm_1.default.SEGMENT_NAME].toLowerCase())) {
    ...
    return (<FormProvider_1.default formID={ONYXKEYS_1.default.FORMS.NETSUITE_CUSTOM_SEGMENT_ADD_FORM} submitButtonText={translate(isEditing ? 'common.confirm' : 'common.next')} onSubmit={handleSubmit} validate={validate} style={[styles.flexGrow1]} submitButtonStyles={[styles.ph5, styles.mb0]} enabledWhenOffline submitFlexEnabled shouldUseScrollView shouldHideFixErrorsAlert addBottomSafeAreaPadding>
            <react_native_1.View style={styles.ph5}>
                ...
                <react_native_1.View style={[styles.flex1, styles.mv3, styles.renderHTML, styles.textDecorationSkipInkNone]}>
                    <RenderHTML_1.default html={`<comment>${Parser_1.default.replace(translate(`workspace.netsuite.import.importCustomFields.customSegments.addForm.${customSegmentRecordType}NameFooter`))}</comment>`}/>
                </react_native_1.View>
            </react_native_1.View>
