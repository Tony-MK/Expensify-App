...
function SageIntacctDatePage({ policy }) {
    const { translate } = (0, useLocalize_1.default)();
    const policyID = policy?.id;
    const styles = (0, useThemeStyles_1.default)();
    const { config } = policy?.connections?.intacct ?? {};
    const { export: exportConfig, pendingFields } = policy?.connections?.intacct?.config ?? {};
    const data = Object.values(CONST_1.default.SAGE_INTACCT_EXPORT_DATE).map((dateType) => ({
        value: dateType,
    ...
    const selectExportDate = (0, react_1.useCallback)((row) => {
        if (row.value !== exportConfig?.exportDate && policyID) {
            (0, SageIntacct_1.updateSageIntacctExportDate)(policyID, row.value, exportConfig?.exportDate);
        }
        goBack();
    }, [exportConfig?.exportDate, policyID, goBack]);
    return (<SelectionScreen_1.default displayName={SageIntacctDatePage.displayName} title="workspace.sageIntacct.exportDate.label" headerContent={headerContent} sections={[{ data }]} listItem={RadioListItem_1.default} onSelectRow={(selection) => selectExportDate(selection)} initiallyFocusedOptionKey={data.find((mode) => mode.isSelected)?.keyForList} policyID={policyID} accessVariants={[CONST_1.default.POLICY.ACCESS_VARIANTS.ADMIN, CONST_1.default.POLICY.ACCESS_VARIANTS.PAID]} featureName={CONST_1.default.POLICY.MORE_FEATURES.ARE_CONNECTIONS_ENABLED} onBackButtonPress={goBack} connectionName={CONST_1.default.POLICY.CONNECTIONS.NAME.SAGE_INTACCT} pendingAction={(0, PolicyUtils_1.settingsPendingAction)([CONST_1.default.SAGE_INTACCT_CONFIG.EXPORT_DATE], pendingFields)} errors={(0, ErrorUtils_1.getLatestErrorField)(config ?? {}, CONST_1.default.SAGE_INTACCT_CONFIG.EXPORT_DATE)} errorRowStyles={[styles.ph5, styles.pv3]} onClose={() => (0, Policy_1.clearSageIntacctErrorField)(policyID, CONST_1.default.SAGE_INTACCT_CONFIG.EXPORT_DATE)}/>);
}
