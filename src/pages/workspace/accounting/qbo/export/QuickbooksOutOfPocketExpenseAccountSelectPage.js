...
function QuickbooksOutOfPocketExpenseAccountSelectPage({ policy }) {
    ...
    const selectExportAccount = (0, react_1.useCallback)((row) => {
        if (row.value.id !== qboConfig?.reimbursableExpensesAccount?.id && policyID) {
            (0, QuickbooksOnline_1.updateQuickbooksOnlineReimbursableExpensesAccount)(policyID, row.value, qboConfig?.reimbursableExpensesAccount);
        }
        goBack();
    ...
    return (<SelectionScreen_1.default policyID={policyID} accessVariants={[CONST_1.default.POLICY.ACCESS_VARIANTS.ADMIN]} featureName={CONST_1.default.POLICY.MORE_FEATURES.ARE_CONNECTIONS_ENABLED} displayName={QuickbooksOutOfPocketExpenseAccountSelectPage.displayName} sections={data.length ? [{ data }] : []} listItem={RadioListItem_1.default} headerContent={<Text_1.default style={[styles.ph5, styles.pb5]}>{description}</Text_1.default>} onBackButtonPress={goBack} onSelectRow={selectExportAccount} initiallyFocusedOptionKey={data.find((mode) => mode.isSelected)?.keyForList} title={title} connectionName={CONST_1.default.POLICY.CONNECTIONS.NAME.QBO} pendingAction={(0, PolicyUtils_1.settingsPendingAction)([CONST_1.default.QUICKBOOKS_CONFIG.REIMBURSABLE_EXPENSES_ACCOUNT], qboConfig?.pendingFields)} errors={(0, ErrorUtils_1.getLatestErrorField)(qboConfig, CONST_1.default.QUICKBOOKS_CONFIG.REIMBURSABLE_EXPENSES_ACCOUNT)} errorRowStyles={[styles.ph5, styles.pv3]} onClose={() => (0, Policy_1.clearQBOErrorField)(policyID, CONST_1.default.QUICKBOOKS_CONFIG.REIMBURSABLE_EXPENSES_ACCOUNT)} listEmptyContent={listEmptyContent} shouldSingleExecuteRowSelect/>);
}
