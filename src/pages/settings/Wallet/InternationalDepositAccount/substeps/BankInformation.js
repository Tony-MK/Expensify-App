...
function BankInformation({ isEditing, onNext, formValues, fieldsMap }) {
    const { translate } = (0, useLocalize_1.default)();
    const styles = (0, useThemeStyles_1.default)();
    const handleSubmit = (0, useInternationalBankAccountFormSubmit_1.default)({
        fieldIds: Object.keys(fieldsMap[CONST_1.default.CORPAY_FIELDS.STEPS_NAME.BANK_INFORMATION]),
        onNext,
        shouldSaveDraft: isEditing,
    });
    const validate = (0, react_1.useCallback)((values) => {
        return (0, utils_1.getValidationErrors)(values, fieldsMap[CONST_1.default.CORPAY_FIELDS.STEPS_NAME.BANK_INFORMATION], translate);
    }, [fieldsMap, translate]);
    ...
    return (<FormProvider_1.default formID={ONYXKEYS_1.default.FORMS.INTERNATIONAL_BANK_ACCOUNT_FORM} submitButtonText={translate(isEditing ? 'common.confirm' : 'common.next')} onSubmit={handleSubmit} validate={validate} style={[styles.flexGrow1, styles.mt3]} submitButtonStyles={[styles.ph5, styles.mb0]} enabledWhenOffline>
            <react_native_1.View style={styles.ph5}>
                <Text_1.default style={[styles.textHeadlineLineHeightXXL, styles.mb6]}>{translate('addPersonalBankAccount.bankInformationStepHeader')}</Text_1.default>
                {Object.values(fieldsMap[CONST_1.default.CORPAY_FIELDS.STEPS_NAME.BANK_INFORMATION])
            .sort((a, b) => CONST_1.default.CORPAY_FIELDS.BANK_INFORMATION_FIELDS.indexOf(a.id) - CONST_1.default.CORPAY_FIELDS.BANK_INFORMATION_FIELDS.indexOf(b.id))
            .map((field, index) => (<react_native_1.View style={getStyle(field, index)} key={field.id}>
                            <InputWrapper_1.default InputComponent={getInputComponent(field)} inputID={field.id} defaultValue={formValues[field.id]} label={field.label + (field.isRequired ? '' : ` (${translate('common.optional')})`)} items={getItems(field)} shouldSaveDraft={!isEditing} renamedInputKeys={{
                street: (0, EmptyObject_1.isEmptyObject)(fieldsMap[CONST_1.default.CORPAY_FIELDS.STEPS_NAME.BANK_INFORMATION]?.bankAddressLine1) ? '' : 'bankAddressLine1',
                street2: (0, EmptyObject_1.isEmptyObject)(fieldsMap[CONST_1.default.CORPAY_FIELDS.STEPS_NAME.BANK_INFORMATION]?.bankAddressLine2) ? '' : 'bankAddressLine2',
                city: (0, EmptyObject_1.isEmptyObject)(fieldsMap[CONST_1.default.CORPAY_FIELDS.STEPS_NAME.BANK_INFORMATION]?.bankCity) ? '' : 'bankCity',
                state: '',
                zipCode: (0, EmptyObject_1.isEmptyObject)(fieldsMap[CONST_1.default.CORPAY_FIELDS.STEPS_NAME.BANK_INFORMATION]?.bankPostal) ? '' : 'bankPostal',
                country: '',
                lat: '',
                lng: '',
            }}/>
