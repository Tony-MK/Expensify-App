"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const react_1 = require("react");
const BaseOnboardingInterestedFeatures_1 = require("./BaseOnboardingInterestedFeatures");
function OnboardingInterestedFeatures(props) {
    return (<BaseOnboardingInterestedFeatures_1.default shouldUseNativeStyles 
    // eslint-disable-next-line react/jsx-props-no-spreading
    {...props}/>);
}
OnboardingInterestedFeatures.displayName = 'OnboardingInterestedFeatures';
exports.default = OnboardingInterestedFeatures;
