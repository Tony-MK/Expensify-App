...
const insertAtCaret = (target, insertedText, maxLength) => {
...
const useHtmlPaste = (textInputRef, preHtmlPasteCallback, isActive = false, maxLength = CONST_1.default.MAX_COMMENT_LENGTH + 1) => {
    /**
     * Set pasted text to clipboard
     * @param {String} text
     */
    const paste = (0, react_1.useCallback)((text) => {
        try {
            const textInputHTMLElement = textInputRef.current;
            if (textInputHTMLElement?.hasAttribute('contenteditable')) {
                insertAtCaret(textInputHTMLElement, text, maxLength);
            }
            else {
                const htmlInput = textInputRef.current;
     ...
     */
    const handlePastedHTML = (0, react_1.useCallback)((html) => {
     ...
     */
    const handlePastePlainText = (0, react_1.useCallback)((event) => {
