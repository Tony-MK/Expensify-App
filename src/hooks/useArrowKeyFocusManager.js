...
const useKeyboardShortcut_1 = require("./useKeyboardShortcut");
 ...
 */
function useArrowKeyFocusManager({ maxIndex, onFocusedIndexChange = () => { }, initialFocusedIndex = 0, 
...
disabledIndexes = CONST_1.default.EMPTY_ARRAY, shouldExcludeTextAreaNodes = true, isActive, itemsPerRow, disableCyclicTraversal = false, allowHorizontalArrowKeys = false, allowNegativeIndexes = false, isFocused = true, setHasKeyBeenPressed, }) {
    const [focusedIndex, setFocusedIndex] = (0, react_1.useState)(initialFocusedIndex);
    const prevIsFocusedIndex = (0, usePrevious_1.default)(focusedIndex);
    const arrowConfig = (0, react_1.useMemo)(() => ({
        excludedNodes: shouldExcludeTextAreaNodes ? ['TEXTAREA'] : [],
        isActive,
    }), [isActive, shouldExcludeTextAreaNodes]);
    const horizontalArrowConfig = (0, react_1.useMemo)(() => ({
        excludedNodes: shouldExcludeTextAreaNodes ? ['TEXTAREA'] : [],
    ...
    const arrowUpCallback = (0, react_1.useCallback)(() => {
    ...
    const arrowDownCallback = (0, react_1.useCallback)(() => {
        if (maxIndex < 0 || !isFocused) {
            return;
        }
        setHasKeyBeenPressed?.();
    ...
    const arrowLeftCallback = (0, react_1.useCallback)(() => {
    ...
    const arrowRightCallback = (0, react_1.useCallback)(() => {
        if (maxIndex < 0 || !allowHorizontalArrowKeys) {
            return;
        }
        const nextIndex = disableCyclicTraversal ? maxIndex : 0;
