...
exports.default = (0, createOnyxDerivedValueConfig_1.default)({
    ...
    compute: ([transactions, violations], { sourceValues, currentValue }) => {
        ...
        if (transactionsUpdates) {
            ...
            transactionsToProcess = Object.keys(transactionViolationsUpdates).map((transactionViolation) => transactionViolation.replace(ONYXKEYS_1.default.COLLECTION.TRANSACTION_VIOLATIONS, ONYXKEYS_1.default.COLLECTION.TRANSACTION));
        }
        ...
        for (const transactionKey of transactionsToProcess) {
            ...
            if (previousReportID && previousReportID !== reportID && reportTransactionsAndViolations[previousReportID]) {
                ...
                if (transactionID) {
                    delete reportTransactionsAndViolations[previousReportID].violations[`${ONYXKEYS_1.default.COLLECTION.TRANSACTION_VIOLATIONS}${transactionID}`];
                }
            }
            if (!reportID) {
                continue;
            ...
            const violationKey = `${ONYXKEYS_1.default.COLLECTION.TRANSACTION_VIOLATIONS}${transactionID}`;
