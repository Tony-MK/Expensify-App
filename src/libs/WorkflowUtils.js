...
const INITIAL_APPROVAL_WORKFLOW = {
    members: [],
    approvers: [],
    availableMembers: [],
    usedApproverEmails: [],
...
function calculateApprovers({ employees, firstEmail, personalDetailsByEmail }) {
    const approvers = [];
    // Keep track of approver emails to detect circular references
    const currentApproverEmails = new Set();
    let nextEmail = firstEmail;
    while (nextEmail) {
...
function convertPolicyEmployeesToApprovalWorkflows({ policy, personalDetails, firstApprover, localeCompare }) {
    ...
    const usedApproverEmails = new Set();
    const personalDetailsByEmail = (0, mapKeys_1.default)(personalDetails, (value, key) => value?.login ?? key);
    ...
    const availableMembers = policy?.approvalMode === CONST_1.default.POLICY.APPROVAL_MODE.BASIC ? sortedApprovalWorkflows?.flatMap((workflow) => workflow.members) : (sortedApprovalWorkflows.at(0)?.members ?? []);
 ...
 */
function convertApprovalWorkflowToPolicyEmployees({ approvalWorkflow, previousEmployeeList, membersToRemove, approversToRemove, type, }) {
    ...
    const pendingAction = type === CONST_1.default.APPROVAL_WORKFLOW.TYPE.CREATE ? CONST_1.default.RED_BRICK_ROAD_PENDING_ACTION.ADD : CONST_1.default.RED_BRICK_ROAD_PENDING_ACTION.UPDATE;
    approvalWorkflow.approvers.forEach((approver, index) => {
        const nextApprover = approvalWorkflow.approvers.at(index + 1);
        const forwardsTo = type === CONST_1.default.APPROVAL_WORKFLOW.TYPE.REMOVE ? '' : (nextApprover?.email ?? '');
    ...
    approvalWorkflow.members.forEach(({ email }) => {
        const submitsTo = type === CONST_1.default.APPROVAL_WORKFLOW.TYPE.REMOVE ? '' : (firstApprover.email ?? '');
...
function updateWorkflowDataOnApproverRemoval({ approvalWorkflows, removedApprover, ownerDetails }) {
