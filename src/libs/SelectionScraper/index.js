...
const getHTMLOfSelection = () => {
    ...
    for (let i = 0; i < selection.rangeCount; i++) {
        ...
        if (clonedSelection.textContent) {
            ...
            if (range.commonAncestorContainer instanceof HTMLElement) {
                ...
                parent = range.commonAncestorContainer.parentNode?.closest(`[${tagAttribute}]`) ?? null;
            }
            ...
            while (parent) {
                ...
                child = cloned;
                parent = parent.parentNode?.closest(`[${tagAttribute}]`) ?? null;
            }
    ...
    const divsToRemove = div.querySelectorAll(`[data-${CONST_1.default.SELECTION_SCRAPER_HIDDEN_ELEMENT}=true]`);
...
const replaceNodes = (dom, isChildOfEditorElement) => {
    ...
    if (dom.type.toString() === 'text' && dom instanceof domhandler_1.DataNode) {
        ...
        if (dom.parent instanceof domhandler_1.Element && dom.parent?.attribs?.[tagAttribute] === 'email-with-break-opportunities') {
...
const getCurrentSelection = () => {
    const domRepresentation = (0, htmlparser2_1.parseDocument)(getHTMLOfSelection());
    ...
    const newHtml = (0, dom_serializer_1.default)(domRepresentation).replace(/<br>\n/g, '<br>');
    return newHtml || '';
};
exports.default = {
    getCurrentSelection,
