...
let globalResolveIsNetworkInterceptorInstalled;
let globalRejectIsNetworkInterceptorInstalled;
const globalIsNetworkInterceptorInstalledPromise = new Promise((resolve, reject) => {
 ...
 */
function networkCacheEntryToResponse({ headers, status, statusText, body }) {
...
function waitForActiveRequestsToBeEmpty() {
    ...
    return new Promise((resolve) => {
        const subscription = react_native_1.DeviceEventEmitter.addListener(ACTIVE_REQUESTS_QUEUE_IS_EMPTY_EVENT, () => {
            subscription.remove();
            resolve();
        });
    });
