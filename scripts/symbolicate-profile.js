...
async function getWorkflowRunArtifact() {
    ...
    const artifact = await GithubUtils_1.default.getArtifactByName(artifactName);
    if (artifact === undefined) {
        throw new Error(`Could not find the artifact ${artifactName}! Are you sure the deploy step succeeded?`);
    }
    return artifact.id;
}
const sourcemapDir = path_1.default.resolve(__dirname, '../.sourcemaps');
function downloadFile(url) {
    ...
    const destination = path_1.default.join(sourcemapDir, `${argsMap.platform}-sourcemap-${appVersion}.zip`);
...
function unpackZipFile(zipPath) {
    ...
    return new Promise((resolve, reject) => {
        fs_1.default.unlink(zipPath, (error) => (error ? reject(error) : resolve()));
    });
}
const localSourceMapPath = path_1.default.join(sourcemapDir, `${appVersion}-${argsMap.platform}.map`);
function renameDownloadedSourcemapFile() {
    const androidName = 'index.android.bundle.map';
    const iosName = 'main.jsbundle.map';
    const downloadSourcemapPath = path_1.default.join(sourcemapDir, argsMap.platform === 'ios' ? iosName : androidName);
...
async function fetchAndProcessArtifact() {
    ...
    const downloadUrl = await GithubUtils_1.default.getArtifactDownloadURL(artifactId);
...
async function runAsyncScript() {
    ...
    if (fs_1.default.existsSync(localSourceMapPath)) {
