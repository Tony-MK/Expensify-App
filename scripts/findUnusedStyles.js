...
class ComprehensiveStylesFinder {
    ...
    findAllStyleDefinitions() {
        const styleFiles = (0, glob_1.globSync)(`src/styles/${STYLE_FILE_EXTENSIONS}`, {
            cwd: this.rootDir,
    ...
    extractStyleKeysFromFile(sourceFile, file) {
        const visit = (node) => {
     ...
     */
    extractStyleDefinitionsFromObject(objectLiteral, sourceFile, file) {
        for (const property of objectLiteral.properties) {
            if (ts.isPropertyAssignment(property) || ts.isMethodDeclaration(property)) {
                const key = TSCompilerUtils_1.default.extractKeyFromPropertyNode(property);
     ...
     * @param file - The relative file path for tracking style definition locations
     */
    extractStyleDefinitionFromNode(node, sourceFile, file) {
        // Look for object literal properties (style definitions)
        if (ts.isPropertyAssignment(node) || ts.isMethodDeclaration(node)) {
            const key = TSCompilerUtils_1.default.extractKeyFromPropertyNode(node);
            if (key && this.isStyleKey(key)) {
                try {
                    let shouldCapture = false;
    ...
    loadAllFileContents() {
        ...
        const sourceFiles = (0, glob_1.globSync)(`src/**/${STYLE_FILE_EXTENSIONS}`, {
            cwd: this.rootDir,
    ...
    analyzeFileForStyleUsage(file, content) {
    ...
    checkSpreadPatternsInStylesFile(content, file) {
        try {
            const sourceFile = ts.createSourceFile(file, content, ts.ScriptTarget.Latest, true);
            const visit = (node) => {
    ...
    handleSpreadExpression(expression) {
    ...
    extractIdentifiersFromExpression(expression) {
        const visit = (node) => {
    ...
    isStyleUsedInContent(key, content, file) {
